# original script by https://github.com/supercrabtree/k
...
k () {
  ...
  # ----------------------------------------------------------------------------
  # default colors
  K_COLOR_DI="0;36"  # di:directory
  K_COLOR_LN="0;34"  # ln:symlink
  ...
  do
    ...
    FILEAGES_TO_COLOR=(
          ...
          3600 246  # < less than an hour old
         86400 242  # < less than 1 day old
        604800 242  # < less than 1 week old
       2419200 238  # < less than 28 days (4 weeks) old
      15724800 238  # < less than 26 weeks (6 months) old
      31449600 234  # < less than 1 year old
      62899200 230  # < less than 2 years old
      )
    ...
    do
      ...
      # --------------------------------------------------------------------------
      if [[ $SYMLINK_TARGET != "" ]]; then SYMLINK_TARGET=$'\e['"$K_COLOR_EX"'m'"→ "$'\e[0m'${SYMLINK_TARGET//$'\e'/\\e}$'\e[0m'; fi
      ...
      if [[ $IS_DIRECTORY == 1 ]]; then
          SPACER="${(l:MAX_LEN[5]:)""}"
          print -r -- "$OWNER $SPACER $DATE_OUTPUT $REPOMARKER $NAME $SYMLINK_TARGET"
      else
          print -r -- "$OWNER $FILESIZE_OUT $DATE_OUTPUT $REPOMARKER $NAME $SYMLINK_TARGET"
      fi
