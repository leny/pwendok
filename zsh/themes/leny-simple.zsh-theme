# oh-my-zsh theme from leny/pwendok - simple version
# started at 16/03/2016
...
# Prompt
PROMPT='%{$reset_color%}
$(prompt_user_style)%n%{$reset_color%}($fg_no_bold[cyan]%40<(…)<%~%<<%{$reset_color%}$(git_prompt_info)$(leny_git_prompt_status)$(dock-compose-status)%{$reset_color%})$(prompt_user_style)
› %{$reset_color%}'

ZSH_THEME_GIT_PROMPT_PREFIX="%{$fg_no_bold[yellow]%}∙%{$fg_no_bold[magenta]%}"
...
ZSH_THEME_DOCKER_PROMPT_PREFIX="%{$fg_no_bold[yellow]%}∙"
ZSH_THEME_DOCKER_PROMPT_NO_MACHINE="%{$fg_no_bold[red]%}no machine"
ZSH_THEME_DOCKER_CONTAINERS_COUNT_PREFIX="%{$fg_no_bold[magenta]%}ctnr%{$reset_color%}:%{$fg_no_bold[green]%}"
ZSH_THEME_DOCKER_CONTAINERS_COUNT_SUFFIX=""
ZSH_THEME_DOCKER_NO_CONTAINERS="%{$fg_no_bold[magenta]%}ctnr%{$reset_color%}:%{$fg_no_bold[yellow]%}0"
